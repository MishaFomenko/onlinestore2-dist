...
export default function BasicTabs() {
  ...
  const fetchUserPath = `${process.env.NEXT_PUBLIC_PUBLIC_IP}:${process.env.NEXT_PUBLIC_SERVER_PORT}/api/userData`;
  const fetchUserAction = 'getuser';
  const fetchUserCollection = 'userdata';
  const fetchUserDocument = user?.uid.toString() || null;
  const fetchUserRequestPath = `${fetchUserPath}?action=${fetchUserAction}&col=${fetchUserCollection}&document=${fetchUserDocument}`;

  const purchasesPath = `${process.env.NEXT_PUBLIC_PUBLIC_IP}:${process.env.NEXT_PUBLIC_SERVER_PORT}/api/userData`;
  const purchasesAction = 'getpurchases';
  const purchasesCollection = 'purchases';
  const purchasesDocumentKey = user?.uid.toString() || null;
  const purchasesRequestPath = `${purchasesPath}?action=${purchasesAction}&col=${purchasesCollection}&document=${purchasesDocumentKey}`;
